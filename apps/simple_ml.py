...
def parse_mnist(image_filesname, label_filename):
    def _read_images(path):
        with gzip.open(path, "rb") as f:
            ...
            if magic != 2051:
                raise ValueError("Unexpected MNIST image magic number")
    ...
    def _read_labels(path):
        with gzip.open(path, "rb") as f:
            ...
            if magic != 2049:
                raise ValueError("Unexpected MNIST label magic number")
            data = np.frombuffer(f.read(), dtype=np.uint8)
            return data.reshape(count)

    return _read_images(image_filesname), _read_labels(label_filename)


def softmax_loss(Z, y_one_hot):
    batch = Z.shape[0]
    log_sum_exp = ndl.logsumexp(Z, axes=(-1,))
    gold = (y_one_hot * Z).sum(axes=(-1,))
    return (log_sum_exp - gold).sum() / batch
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    def _to_tensor(array):
        return ndl.Tensor(array.astype(np.float32))

    for start in range(0, num_examples, batch):
        end = start + batch
        xb = _to_tensor(X[start:end])
        yb = y[start:end]
        one_hot = np.zeros((xb.shape[0], num_classes), dtype=np.float32)
        one_hot[np.arange(xb.shape[0]), yb] = 1.0
        yb_tensor = ndl.Tensor(one_hot)
...
def epoch_general_cifar10(dataloader, model, epoch, loss_fn=nn.SoftmaxLoss(), opt=None):
    ...
    if opt is None:
        ...
        for X, y in dataloader:
            if X.device != device:
                X = ndl.Tensor(X.numpy(), device=device)
            if y.device != device:
                y = ndl.Tensor(y.numpy(), device=device)
            out = model(X)
            loss = loss_fn(out, y)
