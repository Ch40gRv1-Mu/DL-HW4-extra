...
class LanguageModel(nn.Module):
    def __init__(self, embedding_size, output_size, hidden_size, num_layers=1,
        ...
        if seq_model == 'rnn':
            ...
            linear_in = embedding_size
        else:
            raise ValueError(f"Unsupported sequence model: {seq_model}")

        self.linear = nn.Linear(linear_in, output_size, device=device, dtype=dtype)
        ### END YOUR SOLUTION
    ...
    def forward(self, x, h=None):
        ...
        if self.seq_model == 'transformer':
            x, h = self.model(x)
            linear_in = self.embedding_size
        else:
            x, h = self.model(x, h)
            linear_in = self.hidden_size

        x = self.linear(x.reshape((seq_len * bs, linear_in)))
