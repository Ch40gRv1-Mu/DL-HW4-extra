...
#define L (BASE_THREAD_NUM_2D * TILE)
...
void PrintCudaDims(CudaDims dim) {
  std::cout << "Block: " << dim.block.x << " " << dim.block.y << " " << dim.block.z << std::endl;
  std::cout << "Grid: " << dim.grid.x << " " << dim.grid.y << " " << dim.grid.z << std::endl;
}
...
__device__ void PrintCudaVec(CudaVec vec) {
  // CudaVec {1, 2, 3}
  printf("CudaVec {");
...
__device__ void GidToIdx(size_t gid, CudaVec shape, CudaVec* idx) {
  int32_t prod = 1;
...
__device__ size_t CudaVecDot(CudaVec a, CudaVec b, size_t offset) {
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  CudaVec idx;
  idx.size = shape.size;
  GidToIdx(gid, shape, &idx);

  int32_t stride_idx = CudaVecDot(strides, idx, offset);
  out[gid] = a[stride_idx];
  /// END SOLUTION
}
...
__global__ void MatMulRegisterTilingKernel(const scalar_t* A, const scalar_t* B, scalar_t* out,
            size_t M, size_t N, size_t P) {
  ...
  for (size_t i = 0; i < TILE * TILE; ++i) C[i] = 0;

  for (size_t k = 0; k < N; ++k) {
    // fill tiled arrays
    for (size_t i = 0; i < TILE; i++) {
      size_t row_a = xbase * TILE + i;
      size_t col_b = ybase * TILE + i;

      // a[i] = A[xbase * TILE + i][k], b[i] = B[k][ybase * TILE + i]
      if (row_a < M) a[i] = A[row_a * N + k];
      if (col_b < P) b[i] = B[k * P + col_b];
    }
...
__global__ void MatMulSharedMemoryTilingKernel(const scalar_t* A, const scalar_t* B, scalar_t* out,
            size_t M, size_t N, size_t P) {
  
  __shared__ scalar_t sA[L * L], sB[L * L];
  ...
  for (size_t ko = 0; ko < N; ko += L) {
    __syncthreads();
    
    size_t nthreads = blockDim.x * blockDim.y;
    size_t ntasks = L * L / nthreads;
    size_t tid = threadIdx.x * blockDim.y + threadIdx.y;

    // cooperative fetching
    for (size_t j = 0; j < ntasks; ++j) {
      size_t x = (j + ntasks * tid) / L; 
      size_t y = (j + ntasks * tid) % L;

      size_t row_a = xblock * L + x;
      size_t col_b = yblock * L + y;

      // sA[x][y] = A[xblock * L + x][ko + y], sB[x][y] = B[ko + y][yblock * L + y]
      if (x < L && row_a < M && (ko + y) < N) sA[x * L + y] = A[row_a * N + ko + y]; 
      if (x < L && (ko + x) < N && col_b < P) sB[x * L + y] = B[(ko + x) * P + col_b]; 
    }

    __syncthreads();

    for (size_t ki = 0; ki < L; ++ki) {
      size_t k = ko + ki;
      ...
      for (size_t i = 0; i < TILE; ++i) {
        ...
        // a[i] = sA[threadIdx.x * TILE + i][ki], b[i] = sB[ki][threadIdx.y * TILE + i]
        if (row_a < M) a[i] = sA[x * L + ki];
        if (col_b < P) b[i] = sB[ki * L + y];
      }
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  // MatMulRegisterTilingKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, M, N, P);
  MatMulSharedMemoryTilingKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, M, N, P);
  /// END SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  size_t base_idx = gid * reduce_size;

  if (gid >= size) return;

  scalar_t result = a[base_idx];
  for (size_t i = 0; i < reduce_size; ++i) {
    result = max(result, a[base_idx + i]);
  }
